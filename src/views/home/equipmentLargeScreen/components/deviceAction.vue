...
<script lang='ts' setup name='deviceAction'>
...
watch(() => props.activationProductData, (newVal, oldVal) => {
    if (newVal !== oldVal) {
        option.value.series[0].data = []
        option.value.xAxis.data = []
        props.activationProductData.forEach((item: any, index: number) => {
            option.value.series[0].data[index] = item.activeCount
            option.value.xAxis.data[index] = item.activeTime.split("-")[1] + "月"
        })
    }
})
</script>
